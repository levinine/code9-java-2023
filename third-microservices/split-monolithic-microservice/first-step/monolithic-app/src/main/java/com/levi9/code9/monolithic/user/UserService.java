...
public class UserService {
    ...
    public void createUser(User user) {
        if (!userRepository.existsByEmail(user.getEmail())) {
            ...
            notifyUserIfNeeded(user, UserOperation.CREATE);
        } else {
    ...
    public void updateUser(User user) {
        ...
        if (existingUser != null) {
            ...
            notifyUserIfNeeded(user, UserOperation.UPDATE);
        } else {
            throw new AlreadyExistsException(String.format("User with email '%s' already exists", user.getEmail()));
        }
    }
    ...
    private void notifyUserIfNeeded(User user, UserOperation operation) {
        if(user.isReceiveUpdate()) {
            notificationService.sendUserUpdate(user.getEmail(), operation.getText());
        }
    }
